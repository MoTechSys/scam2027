...
function CurrentPeriodCard({ period, className }: { period: CurrentPeriod; className?: string }) {
  const t = useTranslations("academic");
  const f = useFormatter();
  return (
    <Card data-testid="current-period" className={className}>
      <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {period.semester ? (
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            <Badge variant={period.semester.status === "ACTIVE" ? "default" : "outline"}>
          ...
          <p className="text-xs text-muted-foreground">
            {t("current.semester")}: {t("current.none")}
          </p>
        )}
      </CardContent>
    </Card>
...
export function AcademicClient({ data, options, query, counts, period, can }: Props) {
  ...
  const tabCount: Record<AcademicTab, number> = {
    years: counts.years,
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 lg:gap-4">
      <PageTabs
        tabs={visibleTabs.map((id) => ({ id, label: t(`tabs.${id}`), badge: tabCount[id] }))}
        activeTab={data.tab}
        onTabChange={(id) => start(() => router.push(`/academic/${id}`))}
      />

      {can.viewYears && <CurrentPeriodCard period={period} className="shrink-0" />}
      ...
      {data.tab === "years" ? (
        ...
        <CatalogueClient
          ...
          query={query}
          can={can}
          onFilter={(parentId) => setParams({ parentId })}
        />
      )}
    </div>
  );
