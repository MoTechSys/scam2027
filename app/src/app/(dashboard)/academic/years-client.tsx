...
function MenuItems({
  ...
  items: { key: string; label: string; icon: React.ReactNode; onClick: () => void; destructive?: boolean }[];
}) {
  return (
    <>
      {items.map((it, i) => (
...
export function YearsClient({ years, yearOptions, can }: Props) {
  ...
  const yearMenu = (y: YearRow) => {
    ...
    if (can.setCurrent && !y.isCurrent)
      items.push({
        key: "current",
        label: t("actions.setCurrent"),
        icon: <Star className="size-4" aria-hidden />,
        onClick: () => setConfirm({ kind: "currentYear", year: y }),
      });
    if (can.semester) {
      items.push({
        key: "addSem",
        label: t("actions.createSemester"),
        icon: <CalendarPlus className="size-4" aria-hidden />,
        onClick: () => setSemDialog({ semester: null, yearId: y.id }),
      });
      items.push({
        key: "edit",
        label: t("actions.edit"),
        icon: <Pencil className="size-4" aria-hidden />,
        onClick: () => setYearDialog(y),
      });
      if (y.semesters.length === 0)
        items.push({
          key: "delete",
          label: t("actions.delete"),
          icon: <Trash2 className="size-4" aria-hidden />,
          onClick: () => setConfirm({ kind: "deleteYear", year: y }),
          destructive: true,
        });
    }
    return items;
  ...
  const semMenu = (s: SemesterRow) => {
    const items: React.ComponentProps<typeof MenuItems>["items"] = [];
    if (can.setCurrent && !s.isCurrent)
      items.push({
        key: "current",
        label: t("actions.setCurrent"),
        icon: <Star className="size-4" aria-hidden />,
        onClick: () => setConfirm({ kind: "currentSemester", semester: s }),
      });
    if (can.semester) {
      items.push({
        key: "edit",
        label: t("actions.edit"),
        icon: <Pencil className="size-4" aria-hidden />,
        onClick: () => setSemDialog({ semester: s }),
      });
      if (!s.isCurrent && s.offeringCount === 0)
        items.push({
          key: "delete",
          label: t("actions.delete"),
          icon: <Trash2 className="size-4" aria-hidden />,
          onClick: () => setConfirm({ kind: "deleteSemester", semester: s }),
          destructive: true,
        });
    }
    return items;
  ...
  const confirmProps = (c: Confirm) => {
    switch (c.kind) {
      case "deleteYear":
        return {
          title: `${t("actions.delete")}: ${c.year.name}`,
          body: t("confirm.delete"),
          destructive: true,
          run: () => deleteYearAction({ id: c.year.id }),
        };
      case "deleteSemester":
        return {
          title: `${t("actions.delete")}: ${c.semester.name}`,
          body: t("confirm.delete"),
          destructive: true,
          run: () => deleteSemesterAction({ id: c.semester.id }),
        };
      case "currentYear":
        return {
          title: `${t("actions.setCurrent")}: ${c.year.name}`,
          body: t("confirm.setCurrentYear"),
          destructive: false,
          run: () => setCurrentYearAction({ id: c.year.id }),
        };
      case "currentSemester":
        return {
          title: `${t("actions.setCurrent")}: ${c.semester.name}`,
          body: t("confirm.setCurrentSemester"),
          destructive: false,
          run: () => setCurrentSemesterAction({ id: c.semester.id }),
        };
    }
  };
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 lg:gap-4" data-testid="page-shell">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            {years.length > 0 && (
              <Button
                variant="outline"
                onClick={() =>
                  setSemDialog({ semester: null, yearId: years.find((y) => y.isCurrent)?.id ?? years[0]?.id })
                }
      ...
      <ScrollRegion label={t("tabs.years")} className="-mx-1 space-y-4 px-1">
        {years.length === 0 ? (
          ...
          years.map((y) => {
            ...
            return (
              <Card
                ...
                className={y.isCurrent ? "border-primary/60" : undefined}
              >
                          ...
                          >
                            <div className="min-w-0 flex-1 space-y-1">
                              ...
                              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                                <span className="tabular-nums">{range(s.startDate, s.endDate)}</span>
                                {s.registrationOpensAt && s.registrationClosesAt && (
                                  <span>
                                    {t("columns.registration")}:{" "}
                                    <span className="tabular-nums">
                                      {range(s.registrationOpensAt, s.registrationClosesAt)}
                                    </span>
                                  </span>
                                )}
                                <span>
                                  {t("columns.offerings")}:{" "}
                                  <span className="tabular-nums">{s.offeringCount}</span>
                                </span>
                              </div>
                            </div>
                            {sItems.length > 0 && (
      ...
      <YearDialog
        open={yearDialog !== undefined}
        onOpenChange={(o) => !o && setYearDialog(undefined)}
        year={yearDialog ?? null}
      />
      {semDialog && (
        <SemesterDialog
          open
          onOpenChange={(o) => !o && setSemDialog(null)}
          semester={semDialog.semester}
          years={yearOptions}
          defaultYearId={semDialog.yearId}
        />
      )}
      {cp && (
        <ConfirmDialog
