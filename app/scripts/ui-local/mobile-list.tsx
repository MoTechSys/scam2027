...
export function MobileList({ items, emptyMessage, actionsLabel = "إجراءات", className }: MobileListProps) {
  if (items.length === 0) {
    return <div className="flex items-center justify-center py-8 text-sm text-muted-foreground">{emptyMessage}</div>;
  }
  return (
          ...
          <>
            {(item.avatar || item.icon) && (
              <div className="shrink-0">
                {item.avatar ?? <div className="flex size-10 items-center justify-center rounded-full bg-primary/10">{item.icon}</div>}
              </div>
            )}
            ...
            {item.onClick && !item.actions && <ChevronLeft className="size-4 shrink-0 text-muted-foreground rtl:block ltr:rotate-180" aria-hidden />}
          </>
        );
        return (
          <li key={item.id} className="flex items-center gap-3">
            {item.onClick ? (
              <button type="button" onClick={item.onClick} className="flex min-h-11 flex-1 items-center gap-3 px-1 py-3 transition-colors active:bg-accent/50">
                {body}
              </button>
            ) : (
...
export function MobileCard({ children, className, onClick }: MobileCardProps) {
  ...
  if (onClick) {
    return (
      <button type="button" onClick={onClick} className={cn(cls, "block w-full transition-transform active:scale-[0.98]")}>
        {children}
      </button>
    );
  }
  ...
  return (
    <div className={cn("relative", className)}>
      <input
        ...
        className="h-11 w-full rounded-full border border-border bg-muted/50 px-4 text-sm placeholder:text-muted-foreground focus:border-primary/50 focus:outline-none focus:ring-2 focus:ring-primary/20"
      />
    </div>
  );
}
    ...
    <div role="group" className={cn("scrollbar-hide flex gap-2 overflow-x-auto pb-1", className)}>
      {filters.map((filter) => (
        <button
          ...
          className={cn(
            "min-h-11 whitespace-nowrap rounded-full px-3 py-1.5 text-xs transition-all",
            filter.active ? "border border-primary/30 bg-primary/10 font-medium text-primary" : "border border-transparent bg-muted/50 text-muted-foreground",
          )}
        >
          {filter.label}
        </button>
