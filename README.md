...
> **الحالة الحالية (2026-09-06):** **23 / 65 مهمة (35%)** — **P0 مكتمل (16/16)**: أساس Next.js 16 + Prisma + PostgreSQL RLS + Auth.js + RBAC (114 صلاحية) + واجهة Omnitrix + اختبارات (Vitest 167 ✅ · Playwright 72 ✅ سطح مكتب + جوال). **P1 قيد التنفيذ (7/15)**: P1-01..P1-07 (مخطط، مستخدمون، أدوار، بنية أكاديمية، مقررات/شُعب/تسجيل، ملفات، إشعارات). **المهمة التالية: P1-08** (سلة المحذوفات — خطة تنفيذ تفصيلية في `docs/90-handoff/HANDOFF.md` الجلسة 16).
>
...
| [`docs/20-product`](docs/20-product) | المتطلبات FR/NFR، **مصفوفة الصلاحيات (114 رمزًا، 14 فئة — مصدر `permissions.ts` المولَّد)**، حالات الاستخدام |
