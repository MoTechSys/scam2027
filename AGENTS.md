...
- القائمة الجانبية تُظهر فقط: لوحة التحكم، المستخدمون، الأدوار (+ ما يُضاف عند إزالة `phase` من `src/lib/nav/items.ts` لكل وحدة تُبنى).
- `seed.ts` يبذر المستأجر والأدوار والمستخدمين فقط؛ **لا يبذر بنية أكاديمية** بعد (يُوسَّع مع P1-04/05).
